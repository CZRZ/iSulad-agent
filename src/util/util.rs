...
pub fn shim_v2_new(container_id: String, addr: String) -> c_int {
    println!("remote-shim-v2::new::{}:: [{}]", container_id, addr);
    if let Err(e) = new_conn(&container_id, &addr) {
        println!("remote-shim-v2::new::{}:: failed, {}.", container_id, e);
        return -1;
    }

    println!("remote-shim-v2::new::{}:: done.", container_id);
    0
}
...
pub fn shim_v2_shutdown(container_id: String) -> c_int {
    ...
    get_conn(&container_id)
        ...
        .unwrap_or_else(|e| {
            println!("remote-shim-v2::shutdown::{}:: failed, {}.", container_id, e);
            -1
        })
}
...
pub fn shim_v2_resize_pty(container_id: String, exec_id: String, height: u32, width: u32) -> c_int {
    println!("remote-shim-v2::resize_pty::{}:: [{}]", container_id, exec_id);
    get_conn(&container_id)
        .and_then(|client| {
            client
        ...
        .unwrap_or_else(|e| {
            println!("remote-shim-v2::resize_pty::{}:: failed, {}.", container_id, e);
            -1
        })
}
...
pub fn shim_v2_resume(container_id: *const c_char) -> c_int {
    ...
    get_conn(&r_container_id)
        ...
        .unwrap_or_else(|e| {
            println!("remote-shim-v2::resume::{}:: failed, {}.", r_container_id, e);
            -1
        })
}
...
#[no_mangle]
pub fn shim_v2_state(container_id: *const c_char, state: &mut State) -> c_int {
    let r_container_id = to_string(container_id);
    println!("remote-shim-v2::state::{}::", r_container_id);
    get_conn(&r_container_id)
        .and_then(|client| {
            client.state(&r_container_id).map(|container_state| {
                state.copy(container_state);
                println!("remote-shim-v2::state::{}:: done.", r_container_id);
                0
            })
        })
        .unwrap_or_else(|e| {
            println!("remote-shim-v2::state::{}:: failed, {}.", r_container_id, e);
            -1
        })
}
