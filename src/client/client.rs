...
impl State {
    pub fn copy(&mut self, in_obj: State) {
        self.id = String::from(in_obj.id);
        self.pid = in_obj.pid;
        self.status = in_obj.status;
        self.stdin = String::from(in_obj.stdin);
        self.stdout = String::from(in_obj.stdout);
        self.stderr = String::from(in_obj.stderr);
...
pub fn new_conn(container_id: &String, addr: &String) -> Result<()> {
    ...
    let fd = connect_to_socket(false, &path.to_string_lossy())?;
